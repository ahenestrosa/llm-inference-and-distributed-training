"""
AWQ quantization benchmark: Llama 3.1 8B on a single A100.

Uses a pre-quantized AWQ model from Hugging Face.
AWQ (Activation-aware Weight Quantization) preserves salient weights based
on activation magnitudes rather than Hessian computation like GPTQ.

Configuration:
    - 4-bit, group_size=128, zero_point=True, GEMM version
"""
...
# Pre-quantized AWQ model (4-bit, group_size=128)
...
def main() -> None:
    ...
    output_path = results_dir / "results_awq.json"
